...
describe('Application For Webinar', () => {
  ...
  it('Updates the homepage when anything but Hello World is submitted', () => {
    ...
    myApp.click('#submitButton')
    myApp.pause(1000)
    let warningMessage = myApp.getText('#errorWarn')
    expect(warningMessage).to.equal('You should say Hello World')
  })
  it('Clears the warning field when Hello World is submitted', () => {
    myApp.setValue('#valueField', 'Hello World')
    myApp.click('#submitButton')
    myApp.pause(1000)
    let warningMessage = myApp.getText('#errorWarn')
